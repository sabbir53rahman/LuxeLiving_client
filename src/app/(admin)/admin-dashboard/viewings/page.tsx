...
import { CalendarCheck, Trash2, Clock, ChevronDown, Home, MapPin } from "lucide-react";
...
export default function AdminViewingsPage() {
  ...
  const totalPages = data?.meta?.totalPages ?? (total > 0 ? Math.ceil(total / metaLimit) : 0);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("Are you sure you want to delete this viewing appointment?")) return;
    try {
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-border/50 bg-muted/20">
                  <th className="text-left px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Property</th>
                  <th className="text-left px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Buyer Details</th>
                  <th className="text-left px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Assigned Agent</th>
                  <th className="text-left px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Schedule</th>
                  <th className="text-left px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Payments</th>
                  <th className="text-left px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Status</th>
                  <th className="text-right px-8 py-5 font-bold text-muted-foreground uppercase tracking-wider text-xs">Actions</th>
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={7} className="text-center py-20">
                      <div className="flex flex-col items-center gap-2 text-muted-foreground">
                        ...
                        <p className="font-medium">No viewing appointments scheduled yet.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  viewings.map((viewing) => (
                    <tr key={viewing.id} className="group hover:bg-muted/5 transition-all duration-300">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          <div className="h-14 w-14 rounded-2xl overflow-hidden border border-border shadow-sm flex-shrink-0">
                            {viewing.property?.images && viewing.property.images.length > 0 ? (
                               <img 
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                           <div className="h-10 w-10 rounded-full bg-gradient-to-br from-purple-50 to-purple-100 flex items-center justify-center text-purple-600 font-bold border border-purple-200 uppercase">
                             {viewing.buyer?.name.charAt(0) ?? "U"}
                           </div>
                           <div>
                            <p className="font-semibold text-foreground">{viewing.buyer?.name ?? "Unknown Buyer"}</p>
                            <p className="text-xs text-muted-foreground">{viewing.buyer?.email}</p>
                           </div>
                        </div>
                      </td>
                      <td className="px-8 py-5 font-medium text-muted-foreground">
                        {viewing.agent?.name ?? (
                          <span className="text-xs italic opacity-50">Not Assigned</span>
                        )}
                      </td>
                      <td className="px-8 py-5">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2 text-foreground font-semibold">
                            ...
                            {viewing.viewingDate
                              ? new Date(viewing.viewingDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                              : "—"}
                          </div>
                          <p className="text-xs text-muted-foreground pl-6">
                            {viewing.viewingDate
                              ? new Date(viewing.viewingDate).toLocaleDateString(undefined, { dateStyle: 'medium' })
                              : "—"}
                          </p>
                        </div>
                      </td>
                        ...
                        <div className="relative group/select">
                          <select
                            ...
                            className={`appearance-none pl-4 pr-10 py-2 rounded-2xl text-xs font-bold border focus:ring-4 focus:ring-purple-500/10 transition-all cursor-pointer ${
                              statusColors[viewing.status] ?? "bg-gray-100 text-gray-700 border-gray-200"
                            }`}
                          >
                            {STATUSES.map((s) => (
